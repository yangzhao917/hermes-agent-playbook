# Hermes Agent Playbook

> Hermes Agent 使用经验与避坑指南 | 基于 v0.14.0

**仓库：** https://github.com/yangzhao917/hermes-agent-playbook

**维护者：** 杨钊（[@yangzhao917](https://github.com/yangzhao917)）

---

## 内容结构（按场景）

```
hermes-agent-playbook/
├── README.md
├── 飞书集成/          # 文档、待办、日历、Token 操作规范
├── 微信使用/          # 消息队列、意图识别、多任务处理
├── 定时任务/          # cron 配置规范、复盘机制
├── 用户规约/          # 用户偏好、硬规矩、踩坑记录
├── 配置指南/          # 初始化配置、更新、新功能
├── 技能管理/          # Skill 评估原则、推荐清单
└── 脚本/              # 实用脚本
```

---

## 核心文档

| 目录 | 内容 |
|------|------|
| [飞书集成](./飞书集成/README.md) | 文档操作、待办任务、日历操作、Token管理 |
| [微信使用](./微信使用/README.md) | 消息队列机制、意图识别与打断 |
| [定时任务](./定时任务/README.md) | cron 架构规范、复盘总结模板 |
| [用户规约](./用户规约/README.md) | 用户偏好、飞书操作规范、时间计算规范 |
| [配置指南](./配置指南/README.md) | 安装配置、更新、新功能说明 |
| [技能管理](./技能管理/README.md) | Skill 评估原则、推荐技能清单 |

---

## 内容来源

- 飞书文档（持续同步）
- Hermes Agent v0.14.0 官方更新

## License

MIT
