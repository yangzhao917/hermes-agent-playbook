...
def ensure_cron(force=False, dry_run=False):
    ...
    if dry_run:
        print(f"  [dry-run] 将创建 cron job '{job_name}' (每日 23:30 CST)")
        return
...
def main():
    parser = argparse.ArgumentParser(description="daily-review 安装脚本")
