...
# Web Search Skill (Ex-Skill)

> **Supersedes the original `web-search` skill (已替代旧版 `web-search`)** — that one only supported DuckDuckGo via `duckduckgo-search` Python package. This version adds Baidu/Bing via `baidusearch` + Playwright fallback. Keep only this one.
...
5. **`web_extract()` vs browser**: `web_extract()` 不支持 DuckDuckGo backend（`ddgs` 只做搜索），读取 URL 内容需用 `browser_navigate` 或 Playwright 脚本
