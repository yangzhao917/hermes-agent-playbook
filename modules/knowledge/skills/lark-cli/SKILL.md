...
> **JSON 输出格式与解析规范（多行格式化/紧凑单行/前缀）**见 `references/lark-cli-json-parse-quirks.md`
...
### ⚠️ 升级陷阱（EACCES / ENOTEMPTY）

lark-cli 全局安装在 `/usr/lib/node_modules/`，普通用户升级会遇到权限错误和残留临时目录：

```bash
# 正确升级流程（2026-05-28 实测）
sudo rm -rf /usr/lib/node_modules/@larksuite/.cli-*   # 清理残留临时目录（必须先做）
sudo npm install -g @larksuite/cli@latest              # 需要 sudo
lark-cli --version                                      # 验证版本
```

**常见错误：**
- `EACCES: permission denied` → 需要 `sudo`
- `ENOTEMPTY: directory not empty` → 先 `sudo rm -rf /usr/lib/node_modules/@larksuite/.cli-*`

### Token 自动续期

lark-cli 1.0.43+ 支持通过 refresh_token 自动续期 access_token（refresh_token 有效期 ~14 天）。

- `lark-cli auth status` 显示 `tokenStatus: "valid"` 表示 token 有效
- access_token 过期后，调用任意 lark-cli API 命令会触发自动刷新
- 如果 refresh_token 也过期，需要重新 `lark-cli auth login`
- **Cron 脚本中的 token 预检模式**：先调 `lark-cli auth status` 检查，失效时调轻量 API（如 `task tasks list --params '{"page_size":1}'`）触发自动刷新，再重试操作。详见 `daily-review` skill。
...
### ⚠️ Shell CWD 失效导致所有命令报错（2026-05-25 新增）

**触发条件：** 用 `rm -rf` 删除了终端当前所在目录（如 `/tmp/xhs-mcp` 或某 skill 目录），或删除了 `~/.hermes/skills/` 下的某个 skill 目录。

**症状：** 所有后续 `terminal` 命令全部报错 `FileNotFoundError: [Errno 2] No such file or directory: '/home/ubuntu/.hermes/skills/xxx'`，`cd` 无效，`ls` 失效，但 `execute_code` 正常。

**根因：** `terminal` 工具维护持久化 shell 进程，`rm -rf` 删除目录后 shell 的 CWD 变成被删除目录，进程仍有状态，但 `getcwd()` 开始报错。整个 shell session 陷入不可用状态。

**解决路径（按优先级）：**
1. **首选**：`execute_code` 工具使用独立进程池，不受 shell CWD 影响，所有 lark-cli 命令均可用 subprocess 调用
2. **次选**：`terminal` 命令加 `workdir="/home/ubuntu"` 或 `workdir="/"` 强制重置（部分情况有效）
3. **最稳**：用 `bash -c 'cd /home/ubuntu && actual-command'` 包裹（workdir 参数不可靠时）
4. **完全恢复**：新建 terminal session（Hermes 重启或新 session），旧 shell 进程彻底销毁

**验证方法：**
```bash
# terminal 失败时，用 execute_code 替代
python3 -c "import subprocess; r = subprocess.run(['lark-cli', 'task', 'tasks', 'list', '--params', '{\"completed\": false}', '--format', 'json'], capture_output=True, text=True, cwd='/home/ubuntu'); print(r.stdout[:200])"
```

**防呆：** `rm -rf` 操作涉及 skill 目录时，操作前先 `cd /home/ubuntu` 确认 CWD 在安全位置。
